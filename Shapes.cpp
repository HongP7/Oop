...
Ellipse::Ellipse(float cx, float cy, float rx, float ry, float fillOpacity, float strokeOpacity, float strokeWidth, RGB fillRGB, RGB strokeRGB, Transform transform, string fill, string stroke)
    : Shape(fillRGB, strokeRGB, fillOpacity, strokeOpacity, strokeWidth, transform, fill, stroke), cx(cx), cy(cy), rx(rx), ry(ry) {}
void Ellipse::Draw(Graphics& graphics, vector<Defs*>& defs)
{
...
Circle::Circle(float cx, float cy, float r, float fillOpacity, float strokeOpacity, RGB fillRGB, RGB strokeRGB, float strokeWidth, Transform transform, string fill, string stroke)
    : Ellipse(cx, cy, r, r, fillOpacity, strokeOpacity, strokeWidth, fillRGB, strokeRGB, transform, fill, stroke), r(r) {}
...
Rectangle::Rectangle(float x, float y, float width, float height, float fillOpacity, float strokeOpacity, RGB fillRGB, RGB strokeRGB, float strokeWidth, Transform transform, string fill, string stroke)
    : Shape(fillRGB, strokeRGB, fillOpacity, strokeOpacity, strokeWidth, transform, fill, stroke), x(x), y(y), width(width), height(height) {}

void Rectangle::Draw(Graphics& graphics, vector<Defs*>& defs)
{
...
string Polygon::chuanhoa1(string input)
{
...
void Polygon::parsePoints1(string s, vector<pair<float, float>>& allPoints)
{
...
Polygon::Polygon(const string& points, float fillOpacity, float strokeOpacity, RGB fillRGB, RGB strokeRGB, float strokeWidth, Transform transform, string fill, string stroke)
...
void Polygon::Draw(Graphics& graphics, vector<Defs*>& defs)
{
    GraphicsState state = TransformSVG(graphics, transform);
...
string Polyline::chuanhoa2(string input)
{
    for (int i = 0; i < input.length(); i++)
    {
        if (input[i] == ',')
...
void Polyline::parsePoints2(string s, vector<pair<float, float>>& allPoints)
{
...
Polyline::Polyline(const string& points, float fillOpacity, float strokeOpacity, float strokeWidth, RGB fillRGB, RGB strokeRGB, Transform transform, string fill, string stroke)
    : points(points), Shape(fillRGB, strokeRGB, fillOpacity, strokeOpacity, strokeWidth, transform, fill, stroke)
{
    parsePoints2(points, allPoints);
}

void Polyline::Draw(Graphics& graphics, vector<Defs*>& defs)
{
    vector<PointF> pointsArray;
    for (const auto& point : allPoints)
    {
